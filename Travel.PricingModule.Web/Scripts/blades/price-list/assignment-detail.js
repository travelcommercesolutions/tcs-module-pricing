angular.module('travel.pricingModule')
    .controller('travel.pricingModule.assignmentDetailController',
    ['$scope', 'virtoCommerce.catalogModule.catalogs', 'virtoCommerce.pricingModule.pricelists', 'virtoCommerce.pricingModule.pricelistAssignments', 'platformWebApp.bladeNavigationService', 'virtoCommerce.coreModule.common.dynamicExpressionService',
        function ($scope, catalogs, pricelists, assignments, bladeNavigationService, dynamicExpressionService) {
            var blade = $scope.blade;
            var vendoCatalogueId = sessionStorage.getItem('vendoCatalogueId');
    ...
    // actions on load
        ...
        pricelists.search({ take: 1000 }, function (result) {
            ...
            if ($scope.catalogs.length != 0 && vendoCatalogueId != null) {
                $scope.catalogs = $scope.catalogs.filter(c => c.id == vendoCatalogueId);
                $scope.pricelists = $scope.pricelists.filter(function (element) {
                                    return element.assignments.some(function (subElement) {
                                    return subElement.catalogId === vendoCatalogueId
                                    });
                });
                if ($scope.pricelists.filter(c => c.name == blade.parentBlade.currentEntity.name).length < 1) {
                    $scope.pricelists = $scope.pricelists.concat(result.results.filter(c => c.name === blade.parentBlade.currentEntity.name))
