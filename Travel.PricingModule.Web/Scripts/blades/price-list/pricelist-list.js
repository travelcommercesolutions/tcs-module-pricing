angular.module('travel.pricingModule')
    .controller('travel.pricingModule.pricelistListController', ['$scope', 'travel.pricingModule.pricelists', 'platformWebApp.dialogService', 'platformWebApp.uiGridHelper', 'platformWebApp.bladeUtils', '$localStorage',
        function ($scope, pricelists, dialogService, uiGridHelper, bladeUtils, $localStorage) {
            var blade = $scope.blade;
            if (blade.vendor != null) {
                sessionStorage.setItem('selectedVendorId', blade.vendor.id);
                sessionStorage.setItem('vendoCatalogueId', blade.vendoCatalogueId);
            }
            else {
                if (!sessionStorage.getItem('selectedVendorId') != null)
                    sessionStorage.removeItem('selectedVendorId');
                    sessionStorage.removeItem('vendoCatalogueId');
               }
            ...
            blade.refresh = function (parentRefresh) {
                ...
                var selectedVendorId = sessionStorage.getItem('selectedVendorId');
                return pricelists.search(searchCriteria, isAdmin, isUserName, selectedVendorId).then(function (response) {
                    var data = response;
            ...
            $scope.selectNode = function (node, isNew) {
                ...
                var newBlade = {
                    id: 'listItemChild',
                    controller: 'travel.pricingModule.pricelistDetailController',
                    template: 'Modules/$(TCS.PricingModule)/Scripts/blades/price-list/pricelist-detail.tpl.html'
                };
