angular.module('travel.pricingModule')
    .controller('travel.pricingModule.catalogItemSelectController', ['$scope', 'virtoCommerce.catalogModule.catalogs', 'virtoCommerce.catalogModule.listEntries', 'platformWebApp.bladeUtils', 'uiGridConstants', 'platformWebApp.uiGridHelper', 'platformWebApp.ui-grid.extension', '$timeout',
    ...
    blade.refresh = function () {
        ...
        if (!$scope.isCatalogSelectMode()) {
            ...
            catalogs.getCatalogs({}, function (results) {
                ...
                $scope.items = results;
                if ($scope.items.length != 0 && vendoCatalogueId != null) {
                    $scope.items = $scope.items.filter(c => c.id == vendoCatalogueId);
                }
